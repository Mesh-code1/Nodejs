...
const [open, SetOpen]= useState(false);
    return  <AppBar sx={{background:"black"}}>
        <StyledToolbar>
            <SocialBox>
                <Facebook />
                <Instagram />
            ...
            <MenuBox sx={{display:{xs:'none', sm:'none', md:'flex'}}}>
                {MenuItems.map((item)=><Typography sx={{cursor:'pointer', fontSize: "14px"}}>{item.Name}</Typography>)}
                
                
                
            </MenuBox>
            <SearchBox><InputBase placeholder="Search..." sx={{color:"white"}}/>
      ...
      >
        <Box sx={{width:350, height:'90vh'}}>
            {MenuItems.map((item)=><MenuItem sx={{cursor:'pointer', fontSize: "14px"}}>{item.Name}</MenuItem>)}
